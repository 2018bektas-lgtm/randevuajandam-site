...
<body class="text-[#4B5563] antialiased min-h-screen flex flex-col pb-16 lg:pb-0 selection:bg-[#C96A2B] selection:text-white">
    ...
    <div class="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-[#E5E7EB] shadow-[0_-8px_30px_rgba(0,0,0,0.04)] block lg:hidden pb-safe-bottom">
        <div class="grid grid-cols-4 h-16">
            ...
            @else
                <a href="{{ route('frontend.hasta.kayit') }}" class="flex flex-col items-center justify-center gap-1 text-slate-400 hover:text-[#C96A2B] transition-colors {{ request()->routeIs('frontend.hasta.kayit') || request()->routeIs('frontend.hasta.giris') ? 'text-[#C96A2B]' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    <span class="text-[9px] font-bold font-display tracking-wide">Kayıt / Giriş</span>
                </a>
            @endif
        </div>
    </div>
