...
@section('baslik', 'Hekim Kayıt & Ödeme - Randevu Ajandam')
...
<section class="relative bg-[#FAFAFA] py-16 overflow-hidden">
    ...
    <div class="max-w-6xl mx-auto px-6 relative z-10">
        ...
        <form action="{{ route('frontend.hekim.kayit.post') }}" method="POST" id="wizardForm" class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            @csrf
            ...
            <!-- Sağ Sütun: Aşamalı Sihirbaz -->
            <div class="lg:col-span-8 bg-white border border-[#E5E7EB] rounded-3xl p-6 sm:p-8 shadow-sm">
                ...
                <div id="step1" class="wizard-step space-y-6">
                    <h3 class="text-xs font-bold text-[#1F2937] uppercase tracking-wider font-display pb-2 border-b border-[#E5E7EB]">
                        1. Kişisel ve Kurumsal Hesap Bilgileri
                    </h3>
                ...
                <!-- ADIM 3: GÜVENLİ ÖDEME -->
                <div id="step3" class="wizard-step hidden-step grid grid-cols-1 md:grid-cols-12 gap-8">
                    
                    <!-- Form girdileri (Sol kısım) -->
                    <div class="md:col-span-6 space-y-4">
                        <h3 class="text-xs font-bold text-[#1F2937] uppercase tracking-wider font-display pb-2 border-b border-[#E5E7EB]">
                            3. Kart Bilgileri
                        </h3>

                        <!-- Kart Bilgisi Girdileri -->
                        <div class="space-y-3">
                            <!-- Kart Sahibi -->
                            <div>
                    ...
                    <!-- Sanal Kart ve Gönderim (Sağ kısım) -->
                    <div class="md:col-span-6 flex flex-col justify-between space-y-6">
                        ...
                        <div class="space-y-3">
                            <button type="submit"
                                ...
                                Ödemeyi Tamamla ve Üye Ol
                            </button>
                            
                            <div class="flex items-center justify-between gap-4">
                                <button type="button" onclick="prevStep(2)"
                                        class="py-2.5 text-xs text-gray-500 hover:text-gray-800 transition-colors font-bold font-display uppercase tracking-wider select-none cursor-pointer">
                                    ← Bilgileri Düzenle
                                </button>
...
<script>
    ...
    function nextStep(step) {
        ...
        if (step === 2) {
            ...
            if (!passwordRegex.test(password)) {
                if (errorDiv && errorText) {
                    ...
                    alert('Güvenliğiniz için şifreniz en az 8 karakter olmalı; en az bir büyük harf, bir küçük harf, bir sayı ve bir özel karakter (!, @, #, $, %, ^, &, *, vb.) içermelidir.');
                }
                return;
            }
        }

        // Custom validation for step 2: branch selection is required
        if (step === 3) {
            const hiddenInputs = currentContainer.querySelectorAll('input[name="branslar[]"]');
        ...
        setTimeout(() => {
            ...
            targetEl.style.display = (step === 3 && window.innerWidth >= 768) ? 'grid' : 'block';
            
            // Allow display swap before removing class for fade animation
            setTimeout(() => {
    ...
    let isFreePackage = {{ $secilenPaket->aylik_fiyat == 0 ? 'true' : 'false' }};
    ...
    // Card input dynamic update behavior
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Password requirements real-time indicator & Popover Toggle
        ...
        if (passwordInput && passwordRequirements) {
            ...
            passwordInput.addEventListener('input', function() {
                ...
                // Clear inline error message when typing
                const errorDiv = document.getElementById('password-error-message');
                if (errorDiv) {
                    errorDiv.classList.add('hidden');
                }
        ...
        // Retrieve old values from Laravel (if redirecting back on error)
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function toggleSelection(id, name, element) {
            ...
            // Hide branch error message if any selection is made
            const bransErrorDiv = document.getElementById('brans-error-message');
