@php
    ...
    $faturaBekleyen = \Illuminate\Support\Facades\Schema::hasColumn('uyelik_odemeleri', 'fatura_durumu')
        ? \App\Models\UyelikOdeme::where('durum', 'onaylandi')
            ->where(function ($q) {
                $q->where('fatura_durumu', 'bekliyor')->orWhereNull('fatura_durumu');
            })->count()
        : 0;
    $havaleBekleyen = \App\Models\UyelikOdeme::where('durum', 'beklemede')
        ->where(function ($q) {
            $q->where('odeme_yontemi', 'havale')
                ->orWhere('provider', 'banka')
                ->orWhere('provider', 'havale');
        })->count();
    $domainSorunlu = \Illuminate\Support\Facades\Schema::hasTable('domain_orders')
        ? \App\Models\DomainOrder::whereIn('durum', ['failed', 'dns_pending', 'purchasing'])->count()
        : 0;
    ...
    $ysbGroups = [
        [
            ...
            'items' => [
                ...
                ['href' => route('yonetim.uyelikler'), 'match' => 'yonetim.uyelikler', 'label' => 'Üyelikler'],
                [
                    'href' => route('yonetim.doktorlar.meslek-kuyruk'),
                    'match' => 'yonetim.doktorlar.meslek-kuyruk',
                    'label' => 'Meslek kuyruğu',
                ...
                [
                    'href' => route('yonetim.uyelik-odemeleri.index', ['durum' => 'beklemede']),
                    'match' => 'yonetim.uyelik-odemeleri.*',
                    'label' => 'Havale / ödemeler',
                    'badge' => $havaleBekleyen > 0 ? (string) $havaleBekleyen : null,
                ],
                [
                    'href' => route('yonetim.web-siteleri'),
                    'match' => 'yonetim.web-siteleri*',
                    'label' => 'Web / domain',
                    'badge' => $domainSorunlu > 0 ? (string) $domainSorunlu : null,
                ],
        ...
        [
            ...
            'items' => [
                ...
                ['href' => route('yonetim.yoneticiler.index'), 'match' => 'yonetim.yoneticiler.*', 'label' => 'Yöneticiler'],
            ],
        ],
        [
            'id' => 'tanimlar',
        ...
        [
            ...
            'items' => [
                ['href' => route('yonetim.seo'), 'match' => 'yonetim.seo', 'label' => 'SEO ayarları'],
                ['href' => route('yonetim.odeme-ayarlari'), 'match' => 'yonetim.odeme-ayarlari*', 'label' => 'Ödeme ayarları'],
                ['href' => route('yonetim.referanslar.index'), 'match' => 'yonetim.referanslar.*', 'label' => 'Referanslar'],
                ['href' => route('yonetim.two-factor'), 'match' => 'yonetim.two-factor', 'label' => '2FA güvenlik'],
            ],
        ],
    ];

    $ysbSectionLabel = 'Menü';
    $ysbExtraHtml = '';
@endphp
