...
@section('baslik', 'SSS Yönetimi - Randevu Ajandam')
@section('sayfa_baslik', 'SSS Yönetimi')
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-[#E5E7EB]">
        <div>
            <h2 class="text-2xl font-bold font-display text-[#111827] tracking-tight flex items-center gap-2.5">
                ...
                Tüm Sıkça Sorulan Sorular (SSS)
            </h2>
            <p class="text-xs text-[#6B7280] mt-1.5 ml-4">Sistem genelinde hekimlerin eklediği tüm sıkça sorulan soruların listesi.</p>
        </div>
    </div>
    ...
    <div class="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="p-5 border-b border-[#E5E7EB] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:max-w-xl">
                ...
                <div class="relative max-w-xs w-full">
                    ...
                    <input type="text" id="tableSearch" placeholder="Soru veya hekim ara..." 
            ...
            <div class="text-xs text-[#6B7280]">
                Toplam <span class="font-bold text-[#111827]">{{ $faqs->total() }}</span> soru tanımlı.
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-[#E5E7EB]">
                        <th class="px-6 py-4 text-[10px] font-bold text-[#6B7280] uppercase tracking-widest font-display">Soru / Cevap</th>
                        <th class="px-6 py-4 text-[10px] font-bold text-[#6B7280] uppercase tracking-widest font-display">Hekim</th>
                        <th class="px-6 py-4 text-[10px] font-bold text-[#6B7280] uppercase tracking-widest font-display">Görüntülenme Sırası</th>
                ...
                <tbody class="divide-y divide-[#E5E7EB]" id="faqTableBody">
                    @foreach($faqs as $f)
                        <tr class="hover:bg-slate-50/30 transition-colors group">
                            <!-- Question / Answer -->
                            <td class="px-6 py-4">
                                <div class="min-w-0 max-w-md">
                                    <span class="block text-sm font-bold text-[#111827] font-display whitespace-normal leading-snug">{{ $f->soru }}</span>
                                    <span class="block text-[10px] text-[#6B7280] mt-1.5 whitespace-normal leading-relaxed">{!! nl2br(e($f->cevap)) !!}</span>
                                </div>
                            </td>
                            <!-- Doctor -->
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-[#111827] font-display">
                                    {{ $f->doktor->unvan }} {{ $f->doktor->ad_soyad }}
                                </div>
                                <div class="text-[10px] text-gray-500 mt-0.5">{{ $f->doktor->uzmanlik_alani }}</div>
                            </td>
                            <!-- Order -->
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-xs">
                                <form action="{{ route('yonetim.faqs.durum', $f->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-[10px] font-bold tracking-wide transition-all duration-200 cursor-pointer {{ $f->aktif ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100' }}">
                                        <span class="w-1.5 h-1.5 rounded-full {{ $f->aktif ? 'bg-emerald-500 animate-pulse' : 'bg-red-400' }}"></span>
                                        {{ $f->aktif ? 'AKTİF' : 'PASİF' }}
                                    </button>
                                </form>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-right">
                                <form action="{{ route('yonetim.faqs.sil', $f->id) }}" method="POST" class="inline" onsubmit="onayModalAc(event, this, 'Bu sıkça sorulan soruyu silmek istediğinize emin misiniz?');">
                                    @csrf
                                    <button type="submit" 
        ...
        @if($faqs->isEmpty())
            <div class="p-12 text-center text-sm text-[#6B7280] bg-slate-50/30">
                Sistemde kayıtlı sıkça sorulan soru bulunamadı.
            </div>
        @else
            <!-- Pagination Links -->
            @if ($faqs->hasPages())
                <div class="p-5 border-t border-[#E5E7EB] bg-slate-50/30 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <p class="text-xs text-[#6B7280]">
                            <span class="font-semibold text-[#111827]">{{ $faqs->firstItem() ?? 0 }}</span> ile <span class="font-semibold text-[#111827]">{{ $faqs->lastItem() ?? 0 }}</span> arası gösteriliyor (Toplam <span class="font-bold text-[#111827]">{{ $faqs->total() }}</span> kayıttan)
                        </p>
                    </div>
                    <div>
