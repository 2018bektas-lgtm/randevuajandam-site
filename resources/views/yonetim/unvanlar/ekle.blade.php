...
@section('baslik', 'Unvan Ekle - Randevu Ajandam')
@section('sayfa_baslik', 'Unvan Ekle')
...
@section('icerik')
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-[#E5E7EB]">
        <div>
            ...
            <p class="text-xs text-[#6B7280] mt-1.5 ml-4">Sistem için yeni bir mesleki unvan oluşturun.</p>
        </div>
    ...
    <div class="max-w-xl bg-white border border-[#E5E7EB] rounded-2xl p-6 sm:p-8 shadow-sm">
        <form action="{{ route('yonetim.unvanlar.store') }}" method="POST" class="space-y-6">
            @csrf

            <!-- Unvan Adı -->
            <div>
                <label for="ad" class="block text-xs font-bold text-[#4B5563] uppercase tracking-wider mb-2 font-display">Unvan Adı</label>
                <input type="text" name="ad" id="ad" value="{{ old('ad') }}" placeholder="Örn: Prof. Dr." required
