    ...
    <div class="max-w-4xl mx-auto">
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-[#E5E7EB]">
            <div>
                ...
                <p class="text-xs text-[#6B7280] mt-1.5 ml-4">Özellikler katalogdan seçilir; yetkiler ve vitrin listesi otomatik oluşur.</p>
            </div>
        ...
        <div class="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm p-6 sm:p-8">
            <form action="{{ route('yonetim.paketler.store') }}" method="POST">
                @csrf
                @include('yonetim.paketler._form', ['paket' => null, 'ozellikGruplari' => $ozellikGruplari, 'seciliOzellikler' => $seciliOzellikler])
                <div class="flex items-center justify-end gap-3 pt-6 mt-6 border-t border-[#E5E7EB]">
                    ...
                    <button type="submit" class="px-5 py-2.5 rounded-xl bg-[#C96A2B] hover:bg-[#B55A20] text-white font-bold text-sm shadow-sm">Paketi Kaydet</button>
                </div>
            </form>
        </div>
    </div>
