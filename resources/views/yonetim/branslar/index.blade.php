...
@section('baslik', 'Branş Yönetimi - Randevu Ajandam')
@section('sayfa_baslik', 'Branş Yönetimi')
...
@section('icerik')
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-[#E5E7EB]">
        <div>
            <h2 class="text-2xl font-bold font-display text-[#111827] tracking-tight flex items-center gap-2.5">
                ...
                Branşlar / Uzmanlık Alanları
            </h2>
            <p class="text-xs text-[#6B7280] mt-1.5 ml-4">Hekimlerin kayıt olurken ve profillerini düzenlerken seçebileceği uzmanlık alanları.</p>
        </div>
        <div class="flex-shrink-0">
            <a href="{{ route('yonetim.branslar.ekle') }}" 
    ...
    <div class="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="p-5 border-b border-[#E5E7EB] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:max-w-xl">
                ...
                <div class="relative max-w-xs w-full">
                    ...
                    <input type="text" id="tableSearch" placeholder="Branş ara..." 
            ...
            <div class="text-xs text-[#6B7280]">
                Toplam <span class="font-bold text-[#111827]">{{ $branslar->total() }}</span> branş tanımlı.
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-[#E5E7EB]" id="bransTableBody">
                        ...
                        <tr class="hover:bg-slate-50/30 transition-colors group">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-xl bg-[#FFF7ED] text-[#C96A2B] border border-[#E7B58A]/30 flex items-center justify-center font-bold text-xs font-display">
                                        {{ mb_strtoupper(mb_substr($b->ad, 0, 2)) }}
                                    </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-[#4B5563] font-medium">
                                <span class="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-800">
                                    {{ $b->doktorlar->count() }} Hekim
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-right">
                                <div class="inline-flex items-center gap-2">
                                    <!-- Edit Button -->
                                    <a href="{{ route('yonetim.branslar.duzenle', $b->id) }}" 
                                    ...
                                    <!-- Delete Button -->
                                    <form action="{{ route('yonetim.branslar.sil', $b->id) }}" method="POST" class="inline" onsubmit="onayModalAc(event, this, 'Bu branşı silmek istediğinize emin misiniz? Bu branşa bağlı doktorlar bu uzmanlık alanından ayrılacaktır.');">
                                        @csrf
                                        <button type="submit" 
                ...
                <div class="p-5 border-t border-[#E5E7EB] bg-slate-50/30 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <p class="text-xs text-[#6B7280]">
                            <span class="font-semibold text-[#111827]">{{ $branslar->firstItem() ?? 0 }}</span> ile <span class="font-semibold text-[#111827]">{{ $branslar->lastItem() ?? 0 }}</span> arası gösteriliyor (Toplam <span class="font-bold text-[#111827]">{{ $branslar->total() }}</span> kayıttan)
                        </p>
                    </div>
                    <div>
                        <nav class="inline-flex rounded-xl -space-x-px shadow-sm" aria-label="Pagination">
                            <!-- Önceki Sayfa Linki -->
                            @if ($branslar->onFirstPage())
                            ...
                            @else
                                <a href="{{ $branslar->previousPageUrl() }}" class="inline-flex items-center px-3.5 py-2.5 rounded-l-xl border border-[#E5E7EB] bg-white text-xs font-semibold text-[#6B7280] hover:text-[#C96A2B] hover:bg-slate-50 transition-colors">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
                                    </svg>
                                </a>
                            ...
                            <!-- Sayfa Numaraları -->
                            @foreach ($branslar->getUrlRange(1, $branslar->lastPage()) as $page => $url)
                                @if ($page == $branslar->currentPage())
                            ...
                            <!-- Sonraki Sayfa Linki -->
                            @if ($branslar->hasMorePages())
                                <a href="{{ $branslar->nextPageUrl() }}" class="inline-flex items-center px-3.5 py-2.5 rounded-r-xl border border-[#E5E7EB] bg-white text-xs font-semibold text-[#6B7280] hover:text-[#C96A2B] hover:bg-slate-50 transition-colors">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const rows = document.querySelectorAll('#bransTableBody tr');
